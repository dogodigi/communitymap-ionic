...
L.Flickr = L.FeatureGroup.extend({
  ...
  _load: function(data) {
    for (var i = 0; i < data.photos.photo.length; i++) {
      ...
      if(p.tags && (p.tags.indexOf("pollution") > -1 || p.tags.indexOf("waste") > -1)){
        ico = new L.Icon({
          iconUrl: 'images/mosquito.png',
          shadowUrl: null,
    ...
    for(var key in this._layers)
      ks.push(key);
    for(var j = 0; j < ks.length-this.options.maxTotal; j++)
      this.removeLayer(this._layers[ks[j]]);
    //this.fire("loaded");
  },
  ...
  _update: function() {
    ...
    var url = 'https://api.flickr.com/services/rest/?method=flickr.photos.search&bbox='+
      minll.lng+','+minll.lat+','+maxll.lng+','+maxll.lat+'&has_geo=1&format=json&extras=geo,url_t,tags,owner_name,date_upload,license'+
